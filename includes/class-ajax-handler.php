...
class Vibe_Comments_Ajax_Handler {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_vibe_edit_comment',             array($this, 'edit_comment')); 
        add_action('wp_ajax_nopriv_vibe_edit_comment',      array($this, 'edit_comment'));
        add_action('wp_ajax_vibe_search_comments',          array($this, 'search_comments')); 
    ...
    public function load_comments() {
        ...
        $args = array(
            ...
            // newest top-level comments first by default (was ASC).
        ...
        // replies are no longer embedded in the initial payload - only
    ...
    private function format_comment_tree($comment, $depth = 3, $now = null, array $children_map = array(), array $reactions_map = array(), $post_author_id = 0, array $user_reactions_map = array()) {
        ...
        return array(
            ...
            // Q&A mode - both universal truths (same for every
            ...
            // edit window: is_edited is cache-safe (universal truth,
        ...
        try {
            ...
            wp_send_json_success(array(
                'comment' => array(
                    ...
                    'is_pinned' => false,
                    // the author just created this comment, so the
            ...
            wp_send_json_error(array('message' => __('That didn\'t go through. Reload the page and try once more.', 'vibe-comments')));
        }
    }
